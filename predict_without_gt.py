...
if __name__ == "__main__":
    architecture = "dpt"
    encoder = "tu-maxvit_small_tf_512.in1k"
    ...
    # dataset
    data_root = r"/home/zcy/chenrong/segment/dataset_0805/"
    # villages = ["zhangzhou", "wanan", "tuling", "tangdong", "qianhuangtu", "lingshui", "jinshan", "hongcuo", "dongzhang", "daigang", "chengping", "bigu"]

    save_dir = "/home/zcy/chenrong/segment/dataset_0805_segment/"

    for type in os.listdir(data_root):
        type_dir = os.path.join(data_root, type)
        for village in os.listdir(type_dir):
            mask_dir = os.path.join(save_dir, f"{type}", f"test_{village}", "mask")
            os.makedirs(mask_dir, exist_ok=True)

            x_test_dir = os.path.join(type_dir, f"{village}")
            y_test_dir = None
